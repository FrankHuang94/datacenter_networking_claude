...
- **Ciena (WaveLogic)**: Ciena's **WaveLogic** DSPs are widely regarded as best-in-class for reach × capacity. **WaveLogic 5 Extreme (WL5e)** achieves 400G at ~6000 km, 800G at ~3000 km, and 1.2T at shorter reach, using advanced PCS and SD-FEC; **WaveLogic 6 Extreme** now ships at **1.6 Tbps per wavelength** in transponder and MOTR module form, with **WaveLogic 6 Nano** covering compact 800G pluggables. WaveLogic is proprietary to Ciena's systems and a core competitive moat (File 23).
- **Nokia (Photonic Service Engine, PSE)**: Nokia's Bell Labs-derived **PSE** DSPs (PSE-3, PSE-4, PSE-5/6, and successors) power its 1830 optical platforms, with virtual variants (PSE-V) for white-box transponders. Since **Nokia's acquisition of Infinera closed in February 2025**, Nokia also owns the **ICE** line below, giving it two coherent DSP families and an InP PIC fab (File 23).
- **Infinera (ICE) — now part of Nokia**: the **Infinite Capacity Engine** DSPs (ICE6 at ~800G/wavelength, ICE7 toward 1.6T) are paired with Infinera's own **InP photonic integrated circuits (PICs)** — a vertically integrated photonics-plus-DSP approach that was distinctive in the industry and is now the technical core of Nokia's optical portfolio.
...
## Extended Deep Dive: The DCO/ACO Distinction and the Pluggable Power Envelope

A practical distinction in coherent pluggables that shaped the industry's path is **DCO (Digital Coherent Optics) versus ACO (Analog Coherent Optics)**. In an **ACO** module, the optical front end (modulator, coherent receiver) is in the pluggable, but the power-hungry DSP stays on the host line card — splitting the coherent function across the module and the host. In a **DCO** module, the entire coherent function, including the DSP, is integrated into the pluggable, so the module is a self-contained coherent transceiver that plugs into a standard port and presents a clean digital interface to the host. DCO won decisively for datacenter and router applications because it makes coherent "just another pluggable" — the host needs no special coherent line card, just a port that can supply the power — which is exactly what enabled the ZR/ZR+ revolution (above) of putting coherent into routers and switches.

The catch is **power**. Integrating the coherent DSP into the pluggable means the module must dissipate the DSP's 15–25 W within the constrained volume and thermal envelope of a pluggable form factor — which is precisely why coherent pluggables pushed to the larger, higher-power OSFP form factor (and OSFP800 for 800G coherent), and why the power envelope of the pluggable form factors (File 10 above) is the binding constraint on coherent integration. The DCO approach concentrated the coherent function into a self-contained, standardized, host-agnostic module at the cost of demanding a high-power-capable port — a trade-off the industry accepted because the operational simplicity of "coherent as a pluggable" was worth it. This power-envelope constraint on coherent pluggables is the same wall that, for direct-detect optics, drives LPO and CPO (File 13): in both cases, the power of the electronics (DSP for coherent, SerDes for direct-detect) within the pluggable's thermal envelope is the limiting factor, and the responses (OSFP for coherent, LPO/CPO for direct-detect) are different facets of the same power problem.

## Extended Deep Dive: Coherent Goes Short — The Convergence with the Datacenter

A significant trend worth drawing out is the **downward migration of coherent** into shorter and shorter reaches, converging with the datacenter's direct-detect domain. Coherent was historically reserved for long-haul because its complexity and cost were justified only by long distances; direct detection owned everything short. But three forces are pushing coherent shorter: the relentless **cost reduction** of coherent DSPs (each generation cheaper per bit), the **standardization** of coherent pluggables (400G-ZR as a multi-vendor commodity), and the **reach limits of direct detection** at the highest speeds (as per-lane rates rise, even short links become harder for simple direct detection). The result is that coherent now serves metro DCI (80 km), and a debate has emerged about whether coherent could eventually serve even campus and intra-datacenter reaches, displacing direct detection at the high end.

The counter-force is that direct detection remains far cheaper and lower-power for the reaches it can serve, and the bulk of intra-datacenter links (the enormous volume of SR/DR/FR optics) will remain direct-detect for the foreseeable future. The likely outcome is a **moving boundary**: coherent steadily encroaches on the longer datacenter reaches (campus, building-to-building) as its cost falls, while direct detection (and its LPO/CPO evolution) holds the high-volume short reaches. This convergence — coherent and direct-detect technologies meeting at the metro/campus boundary, with the boundary moving as economics shift — is one of the defining dynamics of the optical-transceiver market (File 21), and it blurs the once-sharp line between the "transport" world of coherent and the "datacenter" world of direct detection that the structure of this database (Files 09–12) reflects. The two worlds, long separate, are merging, driven by the same relentless cost and speed scaling that drives everything in datacenter networking.
