...
- **CXL 4.0 (November 2025)** — moves the physical layer to **PCIe 7.0 (128 GT/s)**, doubling bandwidth over the 3.x generation while retaining the 256-byte FLIT format, and introduces **bundled ports** that aggregate links into connections of roughly **1.5 TB/s**. The headline use case is **multi-rack memory pooling** — extending the pool beyond a single chassis or rack, which is where the disaggregation thesis becomes economically interesting (see the pooling-economics section below). Systems are expected in the **late 2026–2027** window.

The versioning history above rewards a moment's attention because it encodes the central frustration of CXL. Five specification generations arrived in six years, each substantially more capable than the last; deployed silicon has consistently trailed by two to three generations. CXL 4.0 doubling bandwidth in 2025 does not help a datacenter whose servers implement CXL 2.0. The specification cadence tracks PCIe's, because CXL rides PCIe's electricals; the *product* cadence tracks server refresh cycles, memory-controller design cycles, and — most binding of all — whether a hyperscaler has convinced itself that pooled memory pays for its latency. That gap, not any technical limitation in the specification, is the reason CXL's story has been slower than its roadmap.
